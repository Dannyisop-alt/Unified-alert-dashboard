...
interface AlertCardProps {
  ...
  onAcknowledgeUpdate?: (alertId: string, acknowledged: boolean) => void;
}
...
export const AlertCard = ({ alert, onAcknowledgeUpdate }: AlertCardProps) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [isAcknowledged, setIsAcknowledged] = useState(alert.acknowledged || false);
  ...
  const handleAcknowledge = async () => {
    ...
    try {
      ...
      if (alert.source === 'Application Logs') {
        await api.markGraylogAlertAsAcknowledged(alert.id, true);
      } else if (alert.source === 'Infrastructure Alerts') {
        await api.markOCIAlertAsAcknowledged(alert.id, true);
      }
      
      setIsAcknowledged(true);
      onAcknowledgeUpdate?.(alert.id, true);
    } catch (error) {
  ...
  return (
    <Card className={`${getSeverityBorderStyle(alert.severity)} hover:shadow-md transition-all relative ${
      isAcknowledged ? 'opacity-50 grayscale' : ''
    }`}>
        ...
        <div className="flex items-start justify-between min-h-0">
            ...
            <div className="flex-1 space-y-2 min-w-0">
              ...
              <div>
                <h3 className={`font-semibold mb-1 ${
                  isAcknowledged ? 'text-gray-500 line-through' : 'text-gray-900'
                }`}>
                ...
                <div className="overflow-hidden">
                  <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
                    <div className={`text-sm break-words ${
                      isAcknowledged ? 'text-gray-400' : 'text-gray-600'
                    }`}>
                    ...
                    {alert.source === 'Infrastructure Alerts' && (alert.compartment || alert.metricName) && (
                      <div className={`mt-2 text-xs space-y-1 ${
                        isAcknowledged ? 'text-gray-400' : 'text-gray-500'
                      }`}>
                        {alert.compartment && (
                          <div>
                    ...
                    {shouldShowExpand && (
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" size="sm" className={`mt-1 p-0 h-auto text-xs hover:text-blue-800 ${
                          isAcknowledged ? 'text-gray-400 hover:text-gray-500' : 'text-blue-600'
                        }`}>
          ...
          <div className="flex items-start gap-3 flex-shrink-0 ml-4">
            ...
            >
              ...
              {isAcknowledging ? 'Acknowledging...' : isAcknowledged ? 'Acknowledged' : 'Acknowledge'}
            </Button>
            <div className={`w-3 h-3 rounded-full ${getSeverityDotStyle(alert.severity)} ${
              isAcknowledged ? 'opacity-50' : ''
            }`}></div>
          </div>
        </div>
